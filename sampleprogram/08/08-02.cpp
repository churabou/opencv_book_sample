...
int main()
{
	...
	for( int i = 0; i < NUM_IMG; i++ ) {
		std::string	fileName = "/data_file/calib_img" +std::to_string(i+1) +".jpg";
	...
	if( fswrite.isOpened() ) {
		write( fswrite, "extrinsic", extr );
		write( fswrite, "intrinsic", inner );
		write( fswrite, "distortion", distort );
	}
